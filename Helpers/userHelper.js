...
module.exports = {
    ...
    addToCart: async (details, userId) => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (userCart) {
                let prodExist = await userCart.products.findIndex(products => products.item == id);
    ...
    deleteCartItem: async (userId, proId) => {
        return new Promise(async (resolve, reject) => {
            let cart = await db.get().collection(collection.CART_COLLECTION).updateOne({
                user: objectId(userId)
            }, {
                $pull: {
                    products: {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (count == -1 && quantity == 1) {
                ...
                db.get().collection(collection.CART_COLLECTION).updateOne({
                    ...
                    $inc: {
                        "products.$.quantity": parseInt(count)
                    }
                }).then((response) => {
    ...
    changePaymentStatus: (id) => {
        return new Promise(async (resolve, reject) => {
            console.log(id);
            let item = await db.get().collection(collection.ORDER_COLLECTION).updateOne({
                _id: objectId(id)
            }, {
                $set: {
                    status: 'placed'
    ...
    modalCloseCase: (orderId) => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.ORDER_COLLECTION).deleteOne({
                _id: objectId(orderId)
            },)
            resolve({state: true})
        })
    ...
    changeProfileDetails: (details) => {
        console.log("obhjjjbzl" + details);
        console.log(details);
        return new Promise(async (resolve, reject) => {
            let password = details.password
            details.password = await bcrypt.hash(password, 10)
            let final = await db.get().collection(collection.USER_COLLECTION).updateOne({
                _id: objectId(details._id)
            }, {
                $set: {
                    name: details.name,
                    email: details.updatedpassword,
                    mobile: details.mobile,
                    password: details.password,
                    photo: details.photostatus
                }
            })
            resolve().toArray()
            console.log(final);
